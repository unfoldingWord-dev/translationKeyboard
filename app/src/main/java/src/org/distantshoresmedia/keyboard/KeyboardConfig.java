...
import org.distantshoresmedia.translationkeyboard.LatinKeyboardView;
import org.distantshoresmedia.translationkeyboard.R;
import org.distantshoresmedia.keyboard.*;
...
public class KeyboardConfig {
    ...
    private KeyboardKeyConfig mShiftKey;
    private KeyboardKeyConfig mAltKey;
    private KeyboardKeyConfig mCtrlKey;
    private KeyboardKeyConfig mMetaKey;
    ...
    private List<KeyboardKeyConfig> mKeys;

    /** List of modifier keys such as Shift & Alt, if any */
    private List<KeyboardKeyConfig> mModifierKeys;
    ...
    public KeyboardConfig(Context context, int defaultHeight, int xmlLayoutResId, int modeId, float kbHeightPercent) {
        ...
        mKeys = new ArrayList<KeyboardKeyConfig>();
        mModifierKeys = new ArrayList<KeyboardKeyConfig>();
    ...
    private KeyboardConfig(Context context, int defaultHeight, int layoutTemplateResId,
        ...
        KeyboardRowConfig row = new KeyboardRowConfig(this);
        ...
        for (int i = start; i != end; i+=step) {
            ...
            final KeyboardKeyConfig key = new KeyboardKeyConfig(KeyboardRowConfig);
            key.x = x;
            key.realX = x;
            key.y = y;
    ...
    private void setEdgeFlags() {
        ...
        int row = 0;
        KeyboardKeyConfig prevKey = null;
        int rowFlags = 0;
        for (KeyboardKeyConfig key : mKeys) {
            ...
            if (prevKey == null || key.x <= prevKey.x) {
                ...
                if (prevKey != null) {
                    ...
                    prevKey.edgeFlags |= KeyboardConfig.EDGE_RIGHT;
                }

                // Set the row flags for the current row.
                rowFlags = 0;
                if (row == 0) rowFlags |= KeyboardConfig.EDGE_TOP;
                if (row == mRowCount - 1) rowFlags |= KeyboardConfig.EDGE_BOTTOM;
                ++row;

                // Mark current key as "left edge"
                keyFlags |= KeyboardConfig.EDGE_LEFT;
            }
            key.edgeFlags = rowFlags | keyFlags;
            prevKey = key;
        }
        // Fix up the last key
        if (prevKey != null) prevKey.edgeFlags |= KeyboardConfig.EDGE_RIGHT;
...
//        }
    ...
    public List<KeyboardKeyConfig> getKeys() {
        return mKeys;
    }

    public List<KeyboardKeyConfig> getModifierKeys() {
        ...
        for (int x = 0; x < gridWidth; x += mCellWidth) {
            for (int y = 0; y < gridHeight; y += mCellHeight) {
                ...
                for (int i = 0; i < mKeys.size(); i++) {
                    final KeyboardKeyConfig key = mKeys.get(i);
    ...
    protected KeyboardRowConfig createRowFromXml(Resources res, XmlResourceParser parser) {
        return new KeyboardRowConfig(res, this, parser);
    }

    protected KeyboardKeyConfig createKeyFromXml(Resources res, KeyboardRowConfig parent, int x, int y,
            XmlResourceParser parser) {
        return new KeyboardKeyConfig(res, parent, x, y, parser);
    }
    ...
    private void loadKeyboard(Context context, XmlResourceParser parser) {
        ...
        float x = 0;
        int y = 0;
        KeyboardKeyConfig key = null;
        KeyboardRowConfig currentRow = null;
